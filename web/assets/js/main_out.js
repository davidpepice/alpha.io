(function(Jquery) {
    ...
    function wsMessage(data) {
        ...
        var data = new Reader(new DataView(data.data), 0, true);
        var packetId = data.getUint8();
        switch (packetId) {
            case 0x10: // update nodes
                ...
                for (var i = 0; i < count; i++) {
                    killer = data.getUint32();
                    killed = data.getUint32();
                ...
                while (true) {
                    ...
                    updSkin = !!(flags & 0x04);
                    updName = !!(flags & 0x08);
                    color = updColor ? bytesToHex(data.getUint8(), data.getUint8(), data.getUint8()) : null;
                    skin = updSkin ? data.getStringUTF8() : null;
                    name = updName ? data.getStringUTF8() : null;
            ...
            case 0x11: // update pos
                camera.target.x = data.getFloat32();
                camera.target.y = data.getFloat32();
                camera.target.scale = data.getFloat32();
            ...
            case 0x15: // draw line
                ...
                break;
            case 0x20: // new cell
                cells.mine.push(data.getUint32());
                break;
            case 0x30: // text list
                leaderboard.items = [];
                ...
                for (i = 0; i < count; ++i)
                    leaderboard.items.push(data.getStringUTF8());
                drawLeaderboard();
            ...
            case 0x31: // ffa list
                ...
                var count = data.getUint32();
                for (i = 0; i < count; ++i) {
                    var isMe = !!data.getUint32();
                    var name = data.getStringUTF8();
            ...
            case 0x32: // pie chart
                ...
                var count = data.getUint32();
                for (i = 0; i < count; ++i)
                    leaderboard.items.push(data.getFloat32());
                drawLeaderboard();
                break;
            case 0x40: // set border
                border.left = data.getFloat64();
                border.top = data.getFloat64();
                border.right = data.getFloat64();
                border.bottom = data.getFloat64();
                ...
                data.getUint32(); // game type
                if (!/MultiOgar|OgarII/.test(data.getStringUTF8()) || stats.pingLoopId) break;
            ...
            case 0x63: // chat message
                var flags = data.getUint8();
                var color = bytesToHex(data.getUint8(), data.getUint8(), data.getUint8());

                var name = data.getStringUTF8();
                name = Cell.prototype.parseName(name).name || EMPTY_NAME;
                var message = data.getStringUTF8();
            ...
            case 0xFE: // server stat
                stats.info = JSON.parse(data.getStringUTF8());
            ...
            lines.push([
                {
                    ...
                    color: settings.darkTheme ? "#E0E" : "#58F"
                }
            ]);
        var width = 0;
        var height = 20 * len + 2;
    ...
    function drawBackgroundSectors() {
        ...
        var fondo = new Image();
        fondo.src = "https://www.solofondos.com/wp-content/uploads/2015/12/los-mejores-paisajes-del-mundo-para-fondo-de-pantalla-3.jpg";
        mainCtx.drawImage(fondo, border.centerX, border.centerY);
    ...
    function drawMinimap() {
        ...
        mainCtx.globalAlpha = 0.4;
        //mainCtx.fillRect(beginX, beginY, width, height);
        mainCtx.fillStyle = settings.darkTheme ? "#FFF" : "#EEE";
        // var minimapaBackground = new Image();
        // minimapaBackground.src = "assets/img/checkerboard.png";
        // mainCtx.drawImage(minimapaBackground,  mainCanvas.width - width, mainCanvas.height - height);
        mainCtx.globalAlpha = 1;
        ...
        if (cells.mine.length) {
            for (var i = 0; i < cells.mine.length; i++) {
                ...
                if (cell) {
                    ...
                    var x = beginX + (camera.target.x + halfWidth) * xScaler;
                    var y = beginY + (camera.target.y + halfHeight) * yScaler;
                    var r = Math.max(cell.s, 800)  * (xScaler + yScaler) / 2;
    ...
    function drawBorders() { 
        if (border === undefined || border.width === undefined) return; 
        if (border.centerX !== 0 || border.centerY !== 0 || !settings.showBorder) return;
        //mainCtx.resetTransform();  
        var targetSizeX = 49122; //byId("targetSizeX").value; 
        // console.log(targetSizeX); 
        var borderAR = border.width / border.height; // aspect ratio
        var width = targetSizeX * borderAR * camera.viewportScale;
        var height = targetSizeX / borderAR * camera.viewportScale;
        var beginX = -14247.7876106195;
        var beginY = -14247.7876106195;
        ...
        mainCtx.beginPath();
        mainCtx.strokeStyle = "#AEA";
        mainCtx.lineWidth = 400; 
        mainCtx.lineCap = "round";
        mainCtx.lineJoin = "round";
        var bordes = new Image();
        mainCtx.globalAlpha = 0.5;
        bordes.src = "assets/img/alpha.png";
        mainCtx.drawImage(bordes, border.centerX - 2048, border.centerY - 2048);
        mainCtx.globalAlpha = 1;
        mainCtx.beginPath(); 
        //toCamera(mainCtx);
        ...
        var borderRigth = border.right;
        mainCtx.moveTo(borderLeft, border.top) ; 
        mainCtx.lineTo(borderRigth, border.top);
        mainCtx.lineTo(borderRigth, border.bottom);
        mainCtx.lineTo(borderLeft, border.bottom);
    ...
    Cell.prototype = {
        ...
        skin: null, jagged: false, food: false,
        ...
        movePoints: function() {
            ...
            for (var i = 0; i < len; ++i) {
                ...
                var newVel = (this.pointsVel[i] + Math.random() - 0.5) * 0.10;
        ...
        drawShape: function(ctx) {
            ...
            for (var i = 0, l = cells.mine.length; i < l; i++)
                if (cells.byId.hasOwnProperty(cells.mine[i])) {
                    cell = cells.byId[cells.mine[i]];
                         if (cell.id == this.id && (~~(this.s * this.s / 100)) < 50000 && cells.mine.length == 1) {
                         //if (!(~~(this.s * this.s / 100)) < 50000 && cells.mine.length == 1) {console.log("una sola bola")}  
                         var arrow = new Image();
                         arrow.src = "assets/img/arrow.png";
                         ctx.save();                         
                         var widthX = canvas.width / 2,
                             heightY = canvas.height / 2;
                         var dx = mouseX - widthX,
                             dy = mouseY - heightY,
                         rotation = Math.atan2(dy, dx);
                         ctx.translate(this.x, this.y);
                         ctx.rotate(rotation);
                         ctx.translate(-this.x, -this.y);
                         ctx.drawImage(arrow, this.x - this.s * 1.4, this.y - this.s * 1.4,
                                this.s * 2.8, this.s * 2.8);
                         ctx.restore();
                    } 
                    break;
                }
             ///////// ANIMATED SKINS
            if (settings.showAnimated && this.id) {
                
            //         var parado = false;
            //         var elId;
            //         function girar(ctx){ 
                         
            //                 var skinSImage = new Image();
            //                 skinSImage.src = "skins/dogeAnimated.png"; //https://i.imgur.com/3cqdfXu.png
                             
            //                 ctx.drawImage(skinSImage, this.x - this.s, this.y - this.s,
            //                     this.s * 2, this.s * 2);
            //                 ctx.restore();
            ...
            // if(this.jagged) { 
            //     var virusImage = document.getElementById("virusImage");
            //     //var virus = virusImage.getContext("2d");
            //     // var virus = new Image();
            //     ctx.save();
            //     //virusImage.src = virusImage;
            //     ctx.clip();
            //     ctx.drawImage(virusImage, this.x - this.s, this.y - this.s,
            //         this.s * 2, this.s * 2);
            //     ctx.restore();
            // }
            // else ctx.fill();
        ...
        drawAnimacion: function (ctx) {
             ...
             var earth = new Image();
                    ...
                    function draw() {
                      ...
                      // La tierra
                      var time = new Date();
                      ctx.rotate(this.x - this.s, this.y - this.s,
                    this.s * 2, this.s * 2);
                      ctx.translate(105,0);
        ...
        drawText: function(ctx) {
            ...
            if (settings.showMass && (cells.mine.indexOf(this.id) !== -1 || cells.mine.length === 0)) {
                ...
                drawText(ctx, true, this.x, y, this.nameSize / 4, this.drawNameSize / 4, mass);
            }
        }
    };
    // function clock(ctx){
    ...
    //         if (key == "a") intervalA = setInterval(function() {
    //            wsSend(UINT8_CACHE[17]);
    //         }, 40);
    //         if (key == "d") intervalD = setInterval(function() {
    //            wsSend(UINT8_CACHE[17]);
    //         }, macroD / 7);    
    ...
    //     else if (key == "a") clearInterval(intervalA);
    //     else if (key == "d") clearInterval(intervalD);
    // }
        ...
        function keyup(event) {
            switch (event.keyCode) {
                ...
                case 87: // W
                    ...
                    break;
                case 81: // Q
                    if (pressed.q) wsSend(UINT8_CACHE[18]);
                    pressed.q = false;
                    break;
                case 69: // E
                    pressed.e = false;
    ...
    window.changeSkin = function(a) {
        byId("skin").value = a;
        var ver = settings.skin = a;
        byId("gallery").hide();
        var visualise = document.getElementById("visualise").innerHTML = c += '<img class="circular" src="./skins/' + name + '.png">';;
    };
