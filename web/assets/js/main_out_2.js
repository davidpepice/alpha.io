            ...
            var tag = document.getElementById("tag").value;
             ...
             for (var i = 0, l = cells.mine.length; i < l; i++)
                 if (cells.byId.hasOwnProperty(cells.mine[i])) {
                     cell = cells.byId[cells.mine[i]];
                     //console.log(cell);
                     if (tag === "1" && this.name === "team1" & cell.id) {
                         //avatar 
                         ctx.save(); // for the clip
                         ctx.clip();
                         var avatar = new Image(); 
                         avatar.src = "assets/img/avatar.png"; //document.getElementById("skins").value; //"https://i.imgur.com/CN5AxRb.png";
                         ctx.drawImage(avatar, this.x - this.s, this.y - this.s,
                             this.s * 2, this.s * 2);
                         ctx.restore();
                          console.log(avatar);
                     }
